...
(() => {
  ...
  class GameScene extends Phaser.Scene {
    ...
    create(){
      ...
      // Extended safeAttachPointer: handles pointercancel, touchcancel, pointerleave, and pointer capture.
      const safeAttachPointer = (id, downCb, upCb) => {
        const el = document.getElementById(id);
        if (!el) return;

        // pointer events (preferred): pointerdown provides pointerId suitable for setPointerCapture
        el.addEventListener('pointerdown', (e) => {
          // Ensure we capture the pointer so moves/cancels remain associated with this element
          try { if (e.pointerId && el.setPointerCapture) el.setPointerCapture(e.pointerId); } catch (err) { /* ignore */ }
          e.preventDefault();
          downCb();
        }, { passive: false });
        ...
        el.addEventListener('pointerup', (e) => {
          ...
          try { if (e.pointerId && el.releasePointerCapture) el.releasePointerCapture(e.pointerId); } catch (err) { /* ignore */ }
          e.preventDefault();
          upCb();
        });

        // pointercancel and pointerleave: ensure we clear state if pointer is canceled or leaves element
        el.addEventListener('pointercancel', (e) => { try { if (e.pointerId && el.releasePointerCapture) el.releasePointerCapture(e.pointerId); } catch (err) {} e.preventDefault(); upCb(); });
        el.addEventListener('pointerleave', (e) => { /* pointer left element bounds */ upCb(); });

        // touch fallback for older browsers (some still dispatch only touch events)
        ...
        el.addEventListener('touchcancel',(e) => { e.preventDefault(); upCb(); });
      };
